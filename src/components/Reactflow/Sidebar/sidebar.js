...
export default () => {
  ...
  return (
    <aside className='h-100 border-2 border-transparent p-4 border-r-slate-200 mr-8'>
      ...
      <div className="dndnode input border-2 rounded-lg border-slate-700 text-center text-xs p-2 mb-2" onDragStart={(event) => onDragStart(event, 'Rectangle')} draggable>
      <strong>RECTANGLE</strong>
      </div>
      <div className="dndnode border-2 rounded-lg border-slate-700 text-center text-xs p-2 mb-2" onDragStart={(event) => onDragStart(event, 'circle')} draggable>
        
        <strong>ROUNDED RECTANGLE</strong>
      </div>
